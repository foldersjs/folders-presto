...
foldersPresto.ls('/', function cb(error, databases) {
  ...
  foldersPresto.ls('/folders', function cb(error, tables) {
    ...
    foldersPresto.ls('/folders/test', function cb(error, metadata) {
      ...
      foldersPresto.cat('/folders/test/columns.md', function cb(error, columns) {
        if (error) {
          console.log('error in cat table columns');
          console.log(error);
        }

        console.log('cat table columns success, fileSize:', columns.size);

        foldersPresto.cat('/folders/test/select.md', function cb(error, records) {
          if (error) {
            console.log('error in cat table records');
            console.log(error);
          }

          console.log('cat table records success, fileSize:', records.size);
        });
      });
    })
